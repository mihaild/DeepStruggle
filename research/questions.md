...
|:---|:---|:---|:---|
| **Does a CE term toward a search policy collapse RL training?** | X4b arms `E3-29-28` / `E3-31-28` / `E3-34-28` / `E3-35-28` | **yes, completely, with or without a healthy pool.** The famous early collapse was mostly pool starvation — `--resume <dir>` gave the run a pool of one it beat 99.7%, and the same config with a working pool scores 90.0% where the starved one scored 24.0% at the identical step. But the healthy-pool arm then collapses too, from 90.0% at 25M to ~11% at 36.5M, both seats. The pool changes *when*, not *whether* | [log/P15_X4b_collapse_is_pool_starvation.md](log/P15_X4b_collapse_is_pool_starvation.md) |
| **Should rollouts be sampled at the policy's own temperature?** | a 2x2: treatment and matched control at two seeds (`E3-33-30`/`E3-30-28`, `E3-36-31`/`E3-37-31`) | **yes.** At the pre-registered 40M point the bands `0.8 1.2 0.7 1.1` beat *their own seed's* control at both seeds — +11.0 pp (z=3.66) and +7.0 pp (z=3.1). Magnitude differs and seed B's advantage stops growing after 60M where seed A reaches 46%. **The mechanism is unknown**: the ending-mix explanation failed its own control, since two controls at different seeds differ on DEFCON-1 endings by ~8 pp, about as much as the conditions do | [log/P15_rollout_temperature.md](log/P15_rollout_temperature.md) |
