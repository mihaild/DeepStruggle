...
|:---|:---|:---|:---|
| **Does a CE term toward a search policy collapse RL training?** | X4b arms `E3-29-28` / `E3-31-28` / `E3-34-28` / `E3-35-28`, rated per seat against `p28_280M` | **partly yes — but the famous collapse was mostly an artifact.** The run that fell to 24.0% at 25M trained against an opponent pool of **one**, beating it 99.7%, because `--resume <dir>` rebuilt the pool from the parent directory; the same config with a working pool reaches **90.0%** at that step. **However** a genuine decline arrives anyway: `E3-35-28` falls 77.5% → 58.0% around 30M with a healthy pool of 12, carrying the documented signature (entropy rising, `search_ce_grad_frac` falling, `kl_div` spiking). So starvation accelerated and deepened a real failure mode rather than inventing it. In flight — re-read at 35M/40M | [log/P15_X4b_collapse_is_pool_starvation.md](log/P15_X4b_collapse_is_pool_starvation.md) |
| **Should rollouts be sampled at the policy's own temperature?** | `E3-33-30` against matched control `E3-30-28` (one behavioural difference), plus a cross-lineage rating at 70M | **yes on this lineage, pending a second seed.** Bands `0.8 1.2 0.7 1.1` beat the default `0.15 0.50 0.10 0.35` at every point after 10M (15.5% vs 4.5% at 40M, z = 3.66), and the strength is **general, not an anchor exploit**: at 70M the arm rates 1438.5, beating `n26_280M` 80.0% and `p29_280M` 87.5% — 280M checkpoints from lineages it never saw. Mechanism is the ending mix: 59.7% of control games end in DEFCON 1 against 28.9%, because sharpening makes one systematic blunder universal. **Caveat:** its opening is degenerate (0.0/4 West Germany, 5.1/1 Yugoslavia, half the anchor's reference overlap) and it wins anyway — so opening quality and strength are separable here. One seed; `E3-36-31` replicates | [log/P15_rollout_temperature.md](log/P15_rollout_temperature.md) |
