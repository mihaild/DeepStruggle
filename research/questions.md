...
|:---|:---|:---|:---|
| **Does a CE term toward a search policy collapse RL training?** | X4b arms `E3-29-28` / `E3-31-28` / `E3-34-28`, rated per seat against `p28_280M` at 25,034,752 steps | **the headline collapse was an artifact.** The run that fell to 24.0% trained against an opponent pool of **one**, beating it 99.7%, because `--resume <dir>` rebuilt the pool from the parent directory. The same config with a working pool reaches **90.0%** at the same step. A milder decline (80.8% → 57.0%, `kl_div` 62.8) survives in `E3-31-28` with a healthy pool and is **still unexplained**. `E3-35-28` is testing whether the gain sustains to 60M | [log/P15_X4b_collapse_is_pool_starvation.md](log/P15_X4b_collapse_is_pool_starvation.md) |
