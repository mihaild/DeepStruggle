...
| **Should rollouts be sampled at the policy's own temperature?** | `E3-33-30` against matched control `E3-30-28`, one behavioural difference | **provisionally yes, and the default bands look wrong.** Bands `0.8 1.2 0.7 1.1` beat the default `0.15 0.50 0.10 0.35` at every point after 10M — 15.5% vs 4.5% against the anchor at 40M, z = 3.66. The mechanism is the ending mix: 59.7% of the control's games end in DEFCON 1 against 28.9%, because sharpening makes one systematic blunder universal. One seed; 80M is the registered second read | [log/P15_rollout_temperature.md](log/P15_rollout_temperature.md) |
