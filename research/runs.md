...
| **E4-61-43, E4-61-44** | Centred per-entity heads (`--ladder-head-center`) on the E4-57 configuration (TF32), seeds 43 and 44, from scratch to 800M, as a pair. Twins of E4-57-43/44, which diverged at 232M / 596M through the drifting country-logit level. Stage 1 of the owner's plan: does it stop the divergence? Quality next | 0 → 800M | `E4-61-4{3,4}_20260925_*` | *running* ([`log/E4_long_runs.md`](log/E4_long_runs.md)) |
| **E4-56-43** | Long run in fp32: the E4-56 configuration (E4-08 recipe, M2d, λ 0.98, pool 0.3/12, `--no-tf32`, snapshots 10M, pool 5M, no z-loss), seed 43, from scratch to 800M, solo. Does fp32 survive where the TF32 runs E4-57-43 (232M) and E4-57-44 (596M) diverged? | 0 → 800M | `E4-56-43_20260925_104626` | [`log/E4_long_runs.md`](log/E4_long_runs.md): reached 800M without diverging, logits to ~9k (where both TF32 runs died); pinned ~125–265M and recovered; plateau ~2,130–2,150 from 400M (E4-57-44's ~2,250); time alone levels off at ~400M |
| **E4-60-43** | Long run with a weaker z-loss: the E4-57 configuration plus `--z-loss-coef 1e-5`, seed 43, from scratch to 800M, solo | 0 → 800M | `E4-60-43_20260925_074456` | [`log/E4_long_runs.md`](log/E4_long_runs.md): reached 800M without diverging (level ~2), but far weaker: 1725 @160M against its twin's 2185, 2036 @800M; all three z-loss runs −170..−460 at 160M. z-loss as built is not a fix |
| **E4-59-43, E4-59-44** | Long runs with z-loss: the E4-57 configuration (E4-08 recipe, M2d, λ 0.98, pool 0.3/12, TF32, snapshots 10M, pool 5M) plus `--z-loss-coef 1e-4`, seeds 43 and 44 (as E4-57-43/44, which diverged at 232M and 596M through the unbounded logit level), from scratch to 800M, as a pair | 0 → 800M | `E4-59-4{3,4}_20260925_*` | [`log/E4_long_runs.md`](log/E4_long_runs.md): stopped at 199M / 196M (owner): level held at ~0.5, but entropy above the twins on both seeds and both leaning USSR (43 pinned twice, recovered) |
| **E4-57-43, E4-57-44** | Long runs: today's default recipe (the E4-57 configuration: E4-08 recipe, M2d, λ 0.98, pool 0.3/12, TF32, snapshots every 10M, pool every 5M), seeds 43 and 44, from scratch to 800M in one run each, as a pair (~5 h). What training time alone buys | 0 → 800M | `E4-57-4{3,4}_20260925_*` | [`log/E4_long_runs.md`](log/E4_long_runs.md): E4-57-43 diverged at 232M (KL to π_ref 0.04 → 1e6 in 8 iterations; critic and logits destroyed), stopped at ~243M. E4-57-44 diverged at 596M (critic first, then KL; logits 39k at 590M), stopped at ~599M. Before dying, E4-57-44 gained to ~400M then flattened: 2234 @240M → 2350 @400M → 2367 @560–590M, the strongest M2d measured (+50 over E4-08-36@240M) |
| **E4-58-40..42** | P26 compile A/B: the E4-57 recipe (TF32) plus `--compile-update max-autotune`, 80M from scratch. Controls E4-57-40..42; rated in one field with fp32 E4-56 | 0 → 80M | `E4-58-4{0,1,2}_20260924_*` | [`log/P26_quick_screen.md`](log/P26_quick_screen.md): arm means fp32 1884, TF32 1974, compile 1882 (late); compile vs TF32 +20 / −71 / −223 (seed 42 pinned 65–80M). Within noise, but not level on every seed: stays opt-in |
