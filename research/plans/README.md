...
|:---|:---|:---|:---|
| **[P25](P25_collapse_robustness.md)** | **current focus.** A training process with the same quality on any seed. All 10 recent collapses lose the US seat, and no slow-π_ref arm has had one ([`../log/E4_collapse_census_per_seat.md`](../log/E4_collapse_census_per_seat.md)). Bench on λ 0.99 from scratch: `--seat-balance`, slow π_ref, `--per-seat-adv-norm` (predicted null); then auto-rewind and a 6-seed acceptance test. **Before P24 and any long run** | 5 arms × 60M, then 6 seeds × 160M | **running** -- steps 0-2 in `9c49329`, bench running |
