...
|:---|:---|:---|:---|
| **[P25](P25_collapse_robustness.md)** | a training process with the same quality on any seed: per-seat signal logging, `--seat-balance`, `--per-seat-adv-norm`, tested on the λ 0.99 stress bench (collapses 2/2 seeds); then a slow-π_ref schedule, auto-rewind and a 6-seed acceptance test. **Before P24 and any long run** | 4 arms × 60M, then 6 seeds × 160M | **running** -- steps 0-2 implemented (`9c49329`), bench queued |
