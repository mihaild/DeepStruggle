...
|:---|:---|:---|:---|
| **[P23](P23_merged_influence_E4_1.md)** | **E4.1**: "influence, first point in X" as one decision, defined as the composition of the two E4 steps it replaces, so it is opt-in per agent and bit-identical to E4 when off. ~11% fewer decisions per game; then the ops budget in the observation (separate approval) before any long run | ~1 day of engineering + ~2 GPU-h of warm-started A/B | **planned, awaiting approval** |
