...
|:---|:---|:---|:---|
| **[P25](P25_collapse_robustness.md)** | **current focus.** A training process with the same quality on any seed. Owner's criterion: a collapse matters only if training dies or is delayed for long. **Before P24 and any long run** | 5 arms × 4 seeds × 80M, then 6 seeds × 160M | **running** -- WoLF closed (collapses on 3 of 4 seeds); the late collapse was the pool resume bug, fixed in `3803d5d` ([`../log/P25_pool_resume_bug.md`](../log/P25_pool_resume_bug.md)). Next: the loop-closing levers on the λ 0.99 bench, seeds 10–13, 80M -- advantage-normaliser floor c 0.8 and 0.5 (3j), one-sided per-seat entropy ceiling (3k), per-seat KL early stop (3l); then 3m seat-gated rescue distillation and step 5's codified rewind + pool gating |
| **[P26](P26_training_throughput.md)** | training throughput after the 09-23 pass (39k -> 70k steps/s, ~8.5 -> ~1.6 cores, arithmetic unchanged). The engine is not the bottleneck. Left: TF32 / bf16 / `torch.compile` for the compute-bound update (numerics changes, P11's gates), faster snapshot probes (~10% of wall), pinned observation copy | per item; 1-3 need a matched A/B | **proposed**, behind P25 |
