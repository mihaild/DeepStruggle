...
| **[P21](P21_architecture_ladder.md)** | build the architecture up from a flat MLP one mechanism at a time — M0 MLP, M1 grouped projections, M2 shared per-entity encoder, M3 cross-attention, M4 pooling last as a *removal*. No graph convolution (owner, 2026-09-19: never showed a benefit, including paired with identity). `per_entity_heads` and `country_identity` sit in a 2x2 against the pooling axis, since both exist to repair it. Parameter-matched, two seeds, simpler wins ties | ~25 arms, ~37 GPU-h | **proposed, not launched** |
