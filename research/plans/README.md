...
|:---|:---|:---|:---|
| **[P25](P25_collapse_robustness.md)** | **current focus.** A training process with the same quality on any seed. All 11 recent collapses lose the US seat; slow π_ref delays one but does not prevent it ([`../log/E4_collapse_census_per_seat.md`](../log/E4_collapse_census_per_seat.md)). Bench on λ 0.99 from scratch: `--seat-balance`, slow π_ref (expected to delay the collapse), `--per-seat-adv-norm` (predicted null); then auto-rewind and a 6-seed acceptance test. **Before P24 and any long run** | 5 arms × 60M, then 6 seeds × 160M | **running** -- bench done: seat balancing partial (+151 and no collapse on seed 5; level on seed 3), slow π_ref from scratch and per-seat norm fail ([`../log/P25_stress_bench.md`](../log/P25_stress_bench.md)). WoLF (3b) on the bench; **queued 2026-09-23: 3c advantage-normaliser floor, 3d per-seat entropy target, 3e KL target with per-seat early stop** — the levers that act on the loop's own closing points (`rollout_buffer.py:382` rescales a vanished signal to unit noise), ~20 min per 60M arm — then 3f seat-gated rescue distillation, and step 5 rewritten as a codified rewind + pool-gating rule |
| **[P23](P23_merged_influence_E4_1.md)** | **E4.1**: "influence, first point in X" as one decision, defined as the composition of the two E4 steps it replaces, so it is opt-in per agent and bit-identical to E4 when off. ~11% fewer decisions per game; then the ops budget in the observation (separate approval) before any long run | ~1 day of engineering + ~2 GPU-h of A/B | **paused** -- both A/B arms collapsed early (US); level with E4 before collapse ([`../log/P23_E4_1_ab.md`](../log/P23_E4_1_ab.md)). Re-run under P25's recipe |
| **[P24](P24_league.md)** | an AlphaStar-style league (main agent, main exploiter reset to the main's early snapshot, league exploiter) with no supervised policy to reset to. A small trainer change (a pool that grows from other runs' directories) plus a driver over `tools/train.py` | 3 concurrent learners; judged at matched **wallclock** | **proposed**, gated on the P23 verdict, the slow-π_ref replicate and [P25](P25_collapse_robustness.md) |
