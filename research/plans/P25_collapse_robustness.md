...
| 3h | **The pool resume bug and the late collapse (E4-48).** A resume recorded the restored pool at step 0, so spacing eviction drained it to 2 pre-resume members and the next resume dropped them (fixed in `3803d5d`). Every continuation from `E4-08-05@160M` ran on that drained pool, and every one collapsed. E4-48 reruns the plain continuation from a repaired state, seed 5 and a seed-11 branch. The bench (from scratch, never resumed) is not affected. | running |
| 3i | **New seeds (6, 7)** for the bench control and E4-41: is the bench collapse, and E4-41's gain, a property of seeds 3 and 5? | running |
