...
| [`country_identity_without_graph_conv.md`](country_identity_without_graph_conv.md) | with `graph_layers=0` the country pathway is permutation-equivariant, so identity is one of only two symmetry breakers — an ablation moves per-country logits by more than the whole across-country spread, but the embedding encodes no map structure |
