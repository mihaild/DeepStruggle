# Agent deficiencies, the decisiveness reward, and the dominance suite

Records §4–§6 of the original `experiments.md`: the first systematic diagnosis of what the agent
gets wrong, measured on the `sp2_pool_off` control and the `dec_turns40` (K=40) arm — `--arch v2`,
78M steps, 512 envs, **pre-E1 engine**, legacy observation. It covers the forced-win floor and why
more training does not lift it (§4–§4.2), the length-scaled terminal reward that was adopted
(§4.3–§4.4), the discovery of the USSR side imbalance and the missing US late-war recovery
(§4.5–§4.7), the dominance suite (§4.8), and the open questions and method stub that followed
(§5–§6). Later entries extend or overturn parts of this: the side imbalance is taken up again in
[`engine_reanchor_and_human_control.md`](engine_reanchor_and_human_control.md) §8 and is worse than
ever at 480M in [`corrected_engine_arms_H_I.md`](corrected_engine_arms_H_I.md) §27; the dominance
numbers for humans are corrected in [`P7_human_bc_warmup.md`](P7_human_bc_warmup.md) §9.3–§9.5; and
§4.2's critic optimism does not reproduce off-distribution (§8.2). **This file is append-only
history** and is not edited to match current belief — the superseded readings are kept where they
were written.
...
## 6. Method notes — moved to [`../method/running_experiments.md`](../method/running_experiments.md)
